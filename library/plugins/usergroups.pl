#-----------------------------------------------------------
# usergroups.pl
# Parse the SAM hive file for group membership info
	...
	if ($key = $root_key->get_subkey($key_path)) {
		...
		if (scalar(@groups) > 0) {
			...
			foreach my $k (keys %grps) {
				...
				::rptMsg(sprintf "%-18s|%-40s","Group Name",$c_val{group_name}." [".$c_val{num_users}."]");
				::rptMsg(sprintf "%-18s|%-40s","Last Updated",::getDateFromEpoch($grps{$k}{LastWrite}));
				::rptMsg(sprintf "%-18s|%-40s","Group Description",$c_val{comment});
				if ($c_val{num_users} == 0) {
					::rptMsg(sprintf "%-18s|%-40s","Users","None");
				}else {
					...
					#::rptMsg(sprintf "%-18s|%-40s","Users","");
					my $counter = 0;
					foreach my $u (keys %users) {
						$counter++;
						::rptMsg(sprintf "%-18s|%-40s","User ".$counter,$u);
					}
