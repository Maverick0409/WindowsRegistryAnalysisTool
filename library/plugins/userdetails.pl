#-----------------------------------------------------------
# userdetails.pl
# Parse the SAM hive file for user membership info
...
my %acb_flags = (#0x0001 => "Account Disabled",
                 ...
                 0x0200 => "Password is not set to expire");
		...
		if (scalar(@user_list) > 0) {
			foreach my $u (@user_list) {
				...
				if ($rid =~ m/^$tag/) {	
					...
					::rptMsg(sprintf "%-20s|%-40s","Username",$v_val{name}." [".$rid."]");
					::rptMsg(sprintf "%-20s|%-40s","Full Name(if any)",$v_val{fullname});
					::rptMsg(sprintf "%-20s|%-40s","User Comment",$v_val{comment});
					#::rptMsg("Account Type    |".$v_val{type});
					::rptMsg(sprintf "%-20s|%-40s","Account Created",::getDateFromEpoch($c_date)) if ($c_date > 0); 
					...
					my $pwdreset;
					my $pwdfail;
					($f_val{last_login_date} == 0) ? ($lastlogin = "Account used actively") : ($lastlogin = ::getDateFromEpoch($f_val{last_login_date}));
					($f_val{pwd_reset_date} == 0) ? ($pwdreset = "Not reset yet") : ($pwdreset = ::getDateFromEpoch($f_val{pwd_reset_date}));
					($f_val{pwd_fail_date} == 0) ? ($pwdfail = "Never") : ($pwdfail = ::getDateFromEpoch($f_val{pwd_fail_date}));
					...
					eval {
						...
						::rptMsg(sprintf "%-20s|%-40s","InternetName",$internet);
					};
					...
					::rptMsg(sprintf "%-20s|%-40s","Password Hint",$pw_hint) unless ($@);
					::rptMsg(sprintf "%-20s|%-40s","Reset Data",$reset_data) unless ($@);
					::rptMsg(sprintf "%-20s|%-40s","Last Login Date",$lastlogin);
					::rptMsg(sprintf "%-20s|%-40s","Password Reset Date",$pwdreset);
					#::rptMsg("Pwd Fail Date   |".$pwdfail);
					#::rptMsg(sprintf "%-18s|%-40s","Login Count",$f_val{login_count});
					#::rptMsg(sprintf "%-18s|%-40s","Embedded RID",$f_val{rid});
					...
					foreach my $flag (keys %acb_flags) {
						::rptMsg(sprintf "%-20s|%-40s","Password Setting",$acb_flags{$flag}) if ($f_val{acb_flags} & $flag);
					}
					#::rptMsg("");
				}
			}
